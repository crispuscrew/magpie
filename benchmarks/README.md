...
- The summed row weights tasks by size, the median row does not, and on the July run one task carried 68% of the total reduction. Quote whichever you like, but say which. Both are on the composite `lines`, so prefer `impl` over either.
